...
void SysTick_Handler(void){
    ...
    if(UART1_data != _null) BUSY=1;
    
    count = (count+1)%3000000;  // this could be commented. it's just for checking the continuity of the main thread.
}
 ...
 *      There are nine specific characters; W,A,S,D,I,1,2,3, and 4.
 ...
 *      1 - set PWM to 100% for forward and backward
 *      2 - set PWM to  75% for forward and backward
 *      3 - set PWM to  50% for forward and backward
 *      4 - set PWM to  25% for forward and backward
 */
void BT_Car_Control(){
    if(BUSY){
            if(UART1_data == 'W'){
            ...
            BUSY=0; // After finished the task, releases the BUSY flag.
        }
        else{
            // Not busy case, do nothing.
            
            // Check if the program is blocking.
            // if not, it should print a lot of #
//            if(count) UART0_OutChar('#');  
        }
}
...
int main( void ) {
    ...
    M1_PWM6_PF2_Init(625,1);    // IR_Transmitter PWM init(40KHz, 0%) PWM_clk_rate = Bus_clk/2 = 50MHz/2 = 25MHz. 
                                // want 40KHz, so 25MHz/40KHz = 625, therefore 625 is 100% with 0%duty cycle.
